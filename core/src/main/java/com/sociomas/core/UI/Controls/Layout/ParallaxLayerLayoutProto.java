...
public class ParallaxLayerLayoutProto extends RelativeLayout {

    private static final int DEFAULT_BASE_OFFSET_DP = 10;
    private static final int DEFAULT_OFFSET_INCREMENT_DP = 5;
    private static final String TAG = "ParallaxLayerLayoutProto";
    ...
    public ParallaxLayerLayoutProto(Context context) {
        super(context);
    }

    public ParallaxLayerLayoutProto(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void updateTranslations(@Size(2) float[] translations) {
        ...
        for (int i = childCount - 1; i >= 0; i--) {
           ...
           // child.setTranslationY((float) (translationsPx[1]*(0.3)));
            //child.getId(R.id.);
            child.setRotationY((float) (translationsPx[0]*(1)));
            child.setRotationX((float) (translationsPx[1]*(-1)));
            /*child.setElevation(15);
            child.setHovered(true);*/
            //Log.v("MyApp",""+translationsPx[0]);
            if (translationsPx[0]<=-10){
                Log.v("MyApp","Menor a -9");
                directionListener.onLeftListener();
            }
            if(translationsPx[0]>=10){
                Log.v("MyApp","Mayor a 9");
    ...
    public interface TranslationUpdater {
        ...
        void subscribe(ParallaxLayerLayoutProto parallaxLayerLayout);
