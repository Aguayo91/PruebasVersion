...
public class TextViewMoney extends android.support.v7.widget.AppCompatTextView {
    ...
    private static final float MIN_PADDING = 5;
    ...
    public TextViewMoney(Context context) {
        super(context);
        init(context, null);
    }

    public TextViewMoney(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public TextViewMoney(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.TextViewMoney,
                0, R.style.MoneyTextViewDefaultStyle);


        try {
            mSymbolSection.text = typedArray.getString(R.styleable.TextViewMoney_symbol);
            String StringmAmount = typedArray.getString(R.styleable.TextViewMoney_amount);
            mGravity = typedArray.getInt(R.styleable.TextViewMoney_gravity, GRAVITY_CENTER_VERTICAL | GRAVITY_CENTER_HORIZONTAL);
            mSymbolGravity = typedArray.getInt(R.styleable.TextViewMoney_symbolGravity, GRAVITY_TOP | GRAVITY_START);
            mDecimalGravity = typedArray.getInt(R.styleable.TextViewMoney_decimalGravity, GRAVITY_TOP);
            mIncludeDecimalSeparator = typedArray.getBoolean(R.styleable.TextViewMoney_includeDecimalSeparator, true);
            mSymbolMargin = typedArray.getDimensionPixelSize(R.styleable.TextViewMoney_symbolMargin, 0);
            mDecimalMargin = typedArray.getDimensionPixelSize(R.styleable.TextViewMoney_decimalMargin, 0);
            mIntegerSection.textSize = typedArray.getDimension(R.styleable.TextViewMoney_baseTextSize, mIntegerSection.textSize);
            mSymbolSection.textSize = typedArray.getDimension(R.styleable.TextViewMoney_symbolTextSize, mIntegerSection.textSize);
            mDecimalSection.textSize = typedArray.getDimension(R.styleable.TextViewMoney_decimalDigitsTextSize, mIntegerSection.textSize);
            mIntegerSection.color = typedArray.getInt(R.styleable.TextViewMoney_baseTextColor, 0);
            mSymbolSection.color = typedArray.getInt(R.styleable.TextViewMoney_symbolTextColor, mIntegerSection.color);
            mDecimalSection.color = typedArray.getInt(R.styleable.TextViewMoney_decimalTextColor, mIntegerSection.color);
            mDecimalSection.drawUnderline = typedArray.getBoolean(R.styleable.TextViewMoney_decimalUnderline, false);
            int styleTypeface = typedArray.getInt(R.styleable.TextViewMoney_typefaceMoney, BOLD);
            selectTypeface(styleTypeface);
            String format = typedArray.getString(R.styleable.TextViewMoney_format);
            String decimalSeparator = typedArray.getString(R.styleable.TextViewMoney_decimalSeparator);
//            String fontPath = typedArray.getString(R.styleable.TextViewMoney_fontPath);
//            if (fontPath != null) {
                
                mTextPaint.setTypeface(selectTypeface(styleTypeface));
//            }
    ...
    private Typeface selectTypeface(int styleTypeface) {
        Typeface typeface ;
        switch (styleTypeface) {
            case BOLD: {
...
//                setTypeface(null, Typeface.NORMAL);
    ...
    private void createTextFromAmount() {
        ...
        if (separatorIndex > -1) {
            mIntegerSection.text = formattedAmount.substring(0, mIncludeDecimalSeparator ? separatorIndex + 1:
                    separatorIndex).replaceAll("\\.(?=.*\\.)", ",");
