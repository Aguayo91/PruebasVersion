...
import com.sociomas.core.WebService.Model.Response.Nomina.RecibosLiberacionResponse;
...
public class NominaDetalleReciboPresenterImpl extends BasePresenterImpl implements NominaDetalleReciboPresenter {

    public static final String TAG = NominaDetalleReciboPresenterImpl.class.getSimpleName();
...
//                                    notifyData(eventResultset);
                                    ...
                                    ViewEvent eventErr = new ViewEvent(ViewEventType.SHOW_ERROR_DIALOG_EVENT_TYPE);
                                    eventErr.getModel().put(ViewEvent.ERROR_TITLE, "Aviso");
                                    eventErr.getModel().put(ViewEvent.ERROR_MSG, "No tienes recibos pendientes de liberar");
                                    notifyData(eventErr);
                                }
                            } else {
                                ViewEvent eventErr = new ViewEvent(ViewEventType.SHOW_ERROR_DIALOG_EVENT_TYPE);
                    ...
                    if (geografiaResponse != null) {
                        if(geografiaResponse.getRecibos() != null) {
                            if (Integer.parseInt(geografiaResponse.getRecibos().get(0).getErrorCode()) == 0) {
                                ...
                                eventErr.getModel().put(ViewEvent.MESSAGE, context.getResources().getString(R.string.server_error));
                                notifyData(eventErr);
                            }
                        } else {
                            ViewEvent eventErr = new ViewEvent(ViewEventType.ERROR_EVENT_TYPE);
